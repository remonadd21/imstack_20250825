...
public class Bank002_2_array_techer {
   public static void main(String[] args) {
      ...
      while(num != 9) {
         ...
         if(num==1) {
        	 	//1. 빈칸인지 확인, 
        	 	// find 빈칸 찾아서 넣을 번호, 
        	 	// 만약 0번째가 0이라면 find는 0
        	 	int find=-1;
        	 	
        	 	// break는 답을 찾으면 멈춤, 없으면 계속 반복, 
        	 	// continue는 다음으로 내려가지 말고, 다시 조건으로 올라가서 확인함.
        	 	for(int i=0; i<id.length; i++) {
        	 		if(id[i]==null) {find=i; break;}
        	 	}
        	 	if(find==-1) { System.out.println("계좌제를 생성할 수 없습니다."); continue;}
        	 	//2. 빈칸에 입력받기
        	 	System.out.println("[1]아이디 입력> "); id[find] = scanner.next();
        	 	System.out.println("[2]비번 입력> "); pass[find] = scanner.next();
        	 	System.out.println("[3]금액 입력> "); balance[find] = scanner.nextDouble();
         }else if(num==2 || num==3 || num==4 || num==5 ) {
        	 	//1. 사용자 인증
        	 	//  아이디를 입력받아, id[0]과 같고, 비밀번호 입력받아 pass[0]과 같다면 찾은 번호는 0
        	 	int find=-1;
        	 	...
        	 	switch(num) {
        	 		case 2: System.out.println("\n[1]아이디 > "+id[find]+
        	 			...
        	 			break;
        	 		case 3: System.out.println("입급할 금액 > "); 
