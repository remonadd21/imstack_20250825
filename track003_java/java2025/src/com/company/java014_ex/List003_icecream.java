...
class IceCreamDTO{
	...
	public String toString() {
		return "IceCreamDTO name=" + name + ", price=(" + price + "원)";
	}
}


public class List003_icecream {
		...
		while(i) {
			System.out.println("❄️🍦 Welcome to the Magical IceCream Land 🍦❄️"
					...
					switch(num) {
						...
						icePrice = scanner.nextInt();
						
						for(int s=0; s<ice.size(); s++) {
							if(ice.get(s).getName().equals(iceNm)) {
								System.out.println("해당하는 제품이 있습니다. 추가가 어렵습니다.");
								find=-1;
								i=false;
								break;
							}
						...
						ice.add(new IceCreamDTO(iceNm, icePrice));
						System.out.println(ice.toString());
						break;
						
						case 2: System.out.println("아이스크림 목록보기");
						
						System.out.print("아이스크림 이름: ");
						iceNm = scanner.next();
						for(IceCreamDTO s: ice) {
							if(iceNm.equals(s.getName())) {
								find=0;
						...
						case 3: System.out.println("아이스크림 삭제하기");
						System.out.print("아이스크림 이름: ");
						iceNm = scanner.next();
						for(int s=0; s<ice.size(); s++) {
							if(ice.get(s).getName().equals(iceNm)) {
						...
						// 찾은 find 지우기
						ice.remove(find);
						System.out.println(iceNm+" 삭제 완료");
						break;
						
						case 4: System.out.println("아이스크림 조회하기");
						System.out.print("아이스크림 이름: ");
						iceNm = scanner.next();
						
						for(IceCreamDTO s: ice) {
							if(iceNm.equals(s.getName())) {
								System.out.println(ice.contains(s));
								if(ice.contains(s)==true) {
									find=0;
									System.out.println("해당하는 아이스크림 이름: "+s.getName()+
											"\n가격: "+s.getPrice()+"원");
									break;
								}
						...
						case 5: System.out.println("현재 아이스크림의 갯수");
						System.out.println("현재 아이스크림의 총 갯수는 "+ice.size()+"개 입니다.");
						break;
						
						
						case 0: System.out.println("아이스크림 가게 문을 닫습니다.");
						break;
						
						
					}
					
					if(find==-1) {System.out.println("해당하는 정보가 없습니다.");}
